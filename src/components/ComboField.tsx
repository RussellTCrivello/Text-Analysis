...
export function ComboField({
  ...
  const filtered = useMemo(
    () => (query ? options.filter((o) => fold(o.value).includes(query) || fold(o.label ?? '').includes(query)) : options),
    [options, query],
  );
  ...
  const rows = useMemo(
    () => [
      ...
      ...(canCreate ? [{ kind: 'create' as const, entry: { value: typed.trim() } as VocabularyEntry }] : []),
    ],
  ...
  const borderColor = error ? 'var(--error)' : open ? 'var(--ring)' : 'var(--border)';
    ...
    <div ref={boxRef} className="relative" style={{ minWidth: 0 }}>
      ...
      >
        <input
          ...
          style={{ background: 'transparent', color: 'var(--fg)', fontFamily: 'var(--font-body)', minWidth: 0 }}
        />
        <button
          type="button"
          tabIndex={-1}
            ...
            return (
              ...
              >
                ...
                {!isCreate && row.entry.builtin && (
                  <span className="text-[9px] uppercase tracking-wide shrink-0" style={{ color: 'var(--muted-fg)' }}>
                    {t.sections.dictionary.builtIn}
                  </span>
                )}
                {!isCreate && count > 0 && (
                  <span className="text-[10px] shrink-0" style={{ color: 'var(--muted-fg)', fontFamily: 'var(--font-mono)' }}>
                    {count}
                  </span>
                )}
      ...
      {open && rows.length === 0 && (
        <div
          ...
          style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', color: 'var(--muted-fg)' }}
        >
...
export function usageMap(rows: Record<string, unknown>[], field: string): Record<string, number> {
