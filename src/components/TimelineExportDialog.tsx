...
import { ExportArrow, ClockIcon, FilterIcon, LayersIcon } from "./icons"
...
export function TimelineExportDialog({
  ...
  return (
    <InfoModal
      isOpen={isOpen}
      title={t.dialogs.exportDialog.title}
      onClose={onClose}
      size="md"
    >
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field
            label={t.dialogs.exportDialog.filename}
            error={filename ? undefined : "Required"}
          >
            <Input
              value={filename}
          ...
          <div className="flex flex-wrap gap-2">
            {COLUMNS.map((c) => (
              <label
                ...
                style={{
                  background: columns.includes(c.key)
                    ? "var(--primary)"
                    : "var(--secondary-bg)",
                  color: columns.includes(c.key)
                    ? "var(--primary-fg)"
                    : "var(--fg)",
                }}
              >
        ...
        >
          {events.length} events will be written as {FORMAT_META[format].label}.
          Document formats (PDF/DOCX/HTML) use the print header configured in
          settings.
        </div>
        ...
        <div className="flex justify-end gap-2">
          ...
          <Btn
            variant="primary"
            onClick={doExport}
            disabled={!events.length || !filename.trim() || !columns.length}
